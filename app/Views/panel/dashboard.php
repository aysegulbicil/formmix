...
<div class="stats-grid dashboard-kpis">
    ...
    <article class="stat-card"><span>Hazırlık bekleyen</span><strong><?= $dashboardMetrics['openOrderCount'] ?></strong><small>Depo veya sevkiyat işlemi gerekiyor</small></article>
...
<?php if ($dashboardMetrics['recentAttentionOrders']): ?><section class="panel-card dashboard-alerts"><div class="report-section__head"><div><h2>İlgilenilecek siparişler</h2><p>Hazırlık, tedarik veya sevkiyat işlemi bekleyen son kayıtlar.</p></div></div><div class="table-wrap"><table class="data-table"><thead><tr><th>ID</th><th>Belge</th><th>Müşteri</th><th>Personel</th><th>Durum</th><th>Tarih</th></tr></thead><tbody><?php foreach ($dashboardMetrics['recentAttentionOrders'] as $row): ?><tr><td data-label="ID"><?= (int) $row['id'] ?></td><td data-label="Belge"><?= esc($row['document_number']) ?></td><td data-label="Müşteri"><?= esc($row['customer_name']) ?></td><td data-label="Personel"><?= esc($row['employee_name']) ?></td><td data-label="Durum"><span class="badge badge--warning"><?= esc($row['status_label']) ?></span></td><td data-label="Tarih"><?= esc(date('d.m.Y', strtotime($row['created_at']))) ?></td></tr><?php endforeach; ?></tbody></table></div></section><?php endif; ?>
<?php endif; ?>
...
<div class="action-grid">
    ...
    <?php if ($user?->can('orders.create') || $user?->can('orders.view-all') || $user?->can('orders.fulfill')): ?><a class="action-card" href="<?= site_url('panel/siparisler') ?>">
        ...
        <div><h3>Teklif ve siparişler</h3><p>Yeni kayıtları ve sipariş süreçlerini izleyin.</p></div><?php if($pendingOrderCount>0): ?><span class="badge badge--warning"><?= esc($pendingOrderCount) ?> açık süreç</span><?php else: ?><span class="action-card__arrow">→</span><?php endif; ?>
    </a><?php endif; ?>
